...
def test_recall_relevant_lessons(
    learning_manager: LearningManager,
) -> None:
    learning_manager.learn_from_correction(
        context="Git workflow",
        agent_action="Suggested main",
        expected_result="Development uses development",
        actual_result="Agent suggested main",
        user_correction="Main is for stable releases.",
        category="git",
        importance=0.9,
        user_confirmed=True,
    )
    ...
    learning_manager.learn_from_correction(
        context="Testing",
        agent_action="Suggested smoke test",
        expected_result="Use regression testing",
        actual_result="Smoke test suggested",
        user_correction="Use regression testing after changes.",
        category="testing",
        importance=0.8,
        user_confirmed=True,
    )

    results = learning_manager.recall_relevant_lessons(
        "Git main development"
    )

    assert len(results) >= 1
    assert results[0]["category"] == "git"
